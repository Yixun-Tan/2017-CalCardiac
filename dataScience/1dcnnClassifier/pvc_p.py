...
class pvc_p(object):
    ...
    def __init__(self, N, T, C):
        # Data Inputs and Data Parameters
        ...
        self.y_in = tf.placeholder(dtype=tf.float32, shape=[
        ...
        self.layer_1_output_size = (
            self.num_samples - self.layer_1_filter_width + 1)
        self.layer_1_output_reshape_size = (
            self.num_samples - self.layer_1_filter_width + 1) * self.layer_1_num_filters
        ...
        self.layer_2_shape = [
            self.layer_1_output_reshape_size, self.network_output_size]
        ...
        self.layer_1_bias = tf.Variable(
            tf.zeros(shape=self.layer_1_bias_shape, dtype=tf.float32), dtype=tf.float32)

        self.layer_2_fc = tf.Variable(tf.random_normal(shape=self.layer_2_shape,
                                                       mean=0, stddev=self.param_std_dev,
                                                       dtype=tf.float32), dtype=tf.float32)
        self.layer_2_bias = tf.Variable(
            tf.zeros(shape=self.layer_2_bias_shape))
        ...
        self.pvc_logits = tf.matmul(
            self.X_1, self.layer_2_fc) + self.layer_2_bias
    ...
    # param data_gen_iterable: on each iteration, should return a pair of dataset batches (X,y):
    #       a training set and a validation set
    def train_n_steps(self, n, data_gen_iterable):
        training_error = []
        validation_error = []
        for (train_batch, validation_batch) in data_gen_iterable(n):
    ...
    def sgd_update(self, X_instance, y_instance):
        E, _ = self.sess.run([self.error, self.train_step], feed_dict={
                             X_in: X_instance, y_in: y_instance})
        return E

    # just returns error without update
    def get_error(self, X_instance, y_instance):
        E = self.sess.run(self.error, feed_dict={
                          X_in: X_instance, y_in: y_instance})
        return E
    ...
    # Tests the forward pass for shape compatibility
    def test_forward(self, N, T, C):
...
###                     Data                                ###
###############################################################
# data_gen will be moved into a separate module.
# Will produce data batches of (training(X,y), validation(X,y)) sets
...
def main_repl():
    # Documentation
    print('Top level REPL for CalCardiac PVC classifier')
    __doc__ = """
Available Commands:
    get:
 ...
 """
    ...
    while True:  # Looop
        ...
        selection = input(PS1)
        # Eval & Print
        first = selection.strip().split(' ')[0].strip()
        try:
            second = selection.strip().split(' ')[1].strip()
        except:
            pass
        ...
        if first == 'get':
            ...
            if second == 'param':  # Parameter setting
                print('not yet implemented')

        elif first == 'train':  # Training
            try:
                n = int(input('Number of steps: '))
                aa.train_n_steps(n, data_source)
            ...
            print('command "%s" not recognized. type "help" for help'.format(selection))
